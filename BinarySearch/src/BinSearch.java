...
public class BinSearch {
    ...
    public static Item find(List<Item> list, long price) {
        ...
        } else if (list.get(0).price == price) {
            return list.get(0);
        } else if (list.get(list.size() - 1).price == price) {
            return list.get(list.size() - 1);
        }

        int middle = list.size() % 2 == 0 ? list.size() / 2 : (list.size() + 1) / 2;
        if(list.get(middle).price == price) {
            return list.get(middle);
        }

        if(list.get(middle - 1).price > price) {
            return find(list.subList(0, middle - 1), price);
        } else {
            return find(list.subList(middle + 1, list.size() - 1), price);
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(find(list, 9).name);
